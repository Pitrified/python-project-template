...
- Replace the generic project description with a real description of what project_name does.
...
- `src/project_name/config/webapp/`
...
If **no**, remove those from `src/project_name/config/webapp/` and the webapp routers/middleware.
...
Replace the sample environment variable names with the real ones needed by project_name.
Remove any variables that are no longer relevant.

## Update `.github/agents/`

Review each agent definition file. Either:
- Update the agent description and commands to reflect project_name, or
...
that are not relevant to project_name. Keep only what will be actively used.

## Update `README.md`

Replace the template boilerplate with a real description of project_name:
