...
1. Configure the following:
   - **Name**: Your app name (e.g., "Project Name Dev")
...
Create a `.env` file at `~/cred/project_name/.env`:
...
uv run python -m project_name.webapp.app
```
...
using the [cdn_load.sh](https://github.com/Pitrified/python-project-template/blob/main/scripts/webapp/cdn_load.sh) script.
...
1. Create a new router in `src/project_name/webapp/routers/`
2. Include it in `main.py`:
   ```python
   from project_name.webapp.routers import new_router
