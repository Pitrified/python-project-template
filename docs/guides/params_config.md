...
|---|---|---|
| **Config** | `src/project_name/config/` | Defines the *shape* of settings as Pydantic models |
| **Params** | `src/project_name/params/` | Loads *actual values* and produces a Config via `to_config()` |
...
- [`SampleConfig`](../../reference/project_name/config/sample_config/) - config side
- [`SampleParams`](../../reference/project_name/params/sample_params/) - params side
...
1. **Single constructor argument**: `env_type: EnvType | None = None`.
   Pass it explicitly from the caller (e.g., `ProjectNameParams`).  When
...
from project_name.params.env_type import (
    EnvLocationType, EnvStageType, EnvType,
    UnknownEnvLocationError, UnknownEnvStageError,
)
from project_name.config.sample_config import NestedModel, SampleConfig
...
`ProjectNameParams` (the top-level singleton) owns all Params instances
...
```python
from project_name.params.env_type import EnvLocationType, EnvStageType, EnvType
